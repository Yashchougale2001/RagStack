...
def retrieve_chunks(collection, query, top_k=3, min_docs=1, similarity_threshold=0.35, domain=None):
    results = query_chunks(collection, query, top_k=top_k, domain=domain)
    ...
    if len(filtered) < min_docs:
        print("⚠️ Low similarity: Not enough relevant chunks.")
        return []
